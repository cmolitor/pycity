...
desyncPerformances = np.zeros([noHorizons, noOfClusterSplittings, max(clusterSize_members)])
...
lt = localtime()
scenario_name = "performances_{}ClS_{}Days_Start{}-".format(noOfClusterSplittings, noHorizons, startingHorizon)
dirResults = "_results_clustering_multiDesync/{}-{:02d}-{:02d}_{:02d}-{:02d}".format(lt[0], lt[1], lt[2], lt[3], lt[4])
...
########################################################################################################################
    ...
    for s in range(noOfClusterSplittings):
        ...
        # desync
        for startBes in range(clusterSize_members[s]):
            desyncPerformances[h-startingHorizon, s, startBes] = clusterList[s].IDA(fromTime=fromTime, startingBes=startBes, criterion=criterion)

        print "\nDay", h, "\tMembers:", clusterSize_members[s], "\n", desyncPerformances[h-startingHorizon, s, 0:clusterSize_members[s]-1]   # for debugging
    ...
    np.save("{}/{}_h-{}.npy".format(dirResults, scenario_name, h), desyncPerformances)
