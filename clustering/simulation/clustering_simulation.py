...
stepSize = 1800
startingHorizon = 137
noHorizons = 365-startingHorizon
criterion = 'energy'
...
########################################################################################################################
        ...
        if includeExecutionTimeLog:
            startTimeCluster = time()
        for c in range(n):
            performances[1, h-startingHorizon, n_index, c] = clusterListFlex[c].IDA(fromTime=fromTime, startingBes=0, criterion=criterion, sort='nomPower')
        if includeExecutionTimeLog:
