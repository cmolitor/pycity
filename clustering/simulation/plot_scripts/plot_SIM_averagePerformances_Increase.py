...
avgIncreasePerS = [ ( np.mean(avgAll[0, :, s]) - np.mean(avgAll[1, :, s]) ) / np.mean(avgAll[0, :, s]) * 100 for s in range(noClusterSplittings) ]
...
rects = ax.bar(ind, avgIncreasePerS[::-1] , width, color='lightskyblue')

# labeling etc
# ------------

ax.set_ylabel('Performance increase in %', labelpad=20)
...
# save file
if savePlot:
    fig_name = os.path.dirname(__file__) + "/../../../../cm116988_BA_Annika_Wierichs/01_LatexDocument/plots/fig_SIM_avgPerformances_Increase"
