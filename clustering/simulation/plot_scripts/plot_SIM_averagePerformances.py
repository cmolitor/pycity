...
avgAll = np.zeros([2, noDays, noClusterSplittings])
for a in range(2):
    for h in range(noDays):
        for s in range(noClusterSplittings):
            performancesArrayTemp = [ performances[a, h, s, c] for c in range(int(noOfClusters[s])) ]
            avgAll[a, h, s] = np.mean(performancesArrayTemp)

avgRanPerS = [ np.mean(avgAll[0, :, s]) for s in range(noClusterSplittings) ]
avgManPerS = [ np.mean(avgAll[1, :, s]) for s in range(noClusterSplittings) ]
...
rectsRan = ax.bar(ind, avgRanPerS[::-1] , width, color='gold')
rectsMan = ax.bar(ind+width, avgManPerS[::-1] , width, color='darkorange')

# labeling etc
# ------------

ax.set_ylabel('Average REC', labelpad=20)
...
# save file
if savePlot:
    fig_name = os.path.dirname(__file__) + "/../../../../cm116988_BA_Annika_Wierichs/01_LatexDocument/plots/fig_SIM_avgPerformances_Random-Managed"
