...
medianIncreasePerS = [ (np.mean(medianAll[0, :, s]) - np.mean(medianAll[1, :, s]) ) / np.mean(medianAll[0, :, s]) * 100 for s in range(noClusterSplittings) ]
...
rects = ax.bar(ind, medianIncreasePerS, width, color='lightskyblue')

# labeling etc
# ------------

ax.set_ylabel('Median REC Increase in %', labelpad=20)
...
# save file
if savePlot:
    fig_name = os.path.dirname(__file__) + "/../../../../cm116988_BA_Annika_Wierichs/01_LatexDocument/plots/fig_SIM_medianPerformances_Increase"
