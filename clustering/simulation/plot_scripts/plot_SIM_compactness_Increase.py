...
compactnessAll = np.zeros([2, noDays, noClusterSplittings])
meanAll = np.zeros([2, noDays, noClusterSplittings])
for a in range(2):
...
compactnessIncreaseAvgPerS = [ (np.mean(compactnessAll[0, :, s]) - np.mean(compactnessAll[1, :, s])) / np.mean(compactnessAll[0, :, s]) * 100 for s in range(noClusterSplittings) ]
...
rects = ax.bar(ind, compactnessIncreaseAvgPerS[::-1] , width, color='lightskyblue')

# labeling etc
# ------------

ax.set_ylabel('Compactness increase in %', labelpad=20)
...
fig.subplots_adjust(top=0.95, bottom=0.175, left=0.15, right=0.85)
...
# save file
if savePlot:
    fig_name = os.path.dirname(__file__) + "/../../../../cm116988_BA_Annika_Wierichs/01_LatexDocument/plots/fig_SIM_compactness_Increase"
