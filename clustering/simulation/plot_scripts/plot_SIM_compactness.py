...
compactnessManAvgPerS = [ np.mean(compactnessAll[1, :, s]) for s in range(noClusterSplittings) ]
...
rectsRan = ax.bar(ind, compactnessRanAvgPerS[::-1] , width, color='lightcoral')
rectsMan = ax.bar(ind+width, compactnessManAvgPerS[::-1] , width, color='yellowgreen')
...
# save file
if savePlot:
    fig_name = os.path.dirname(__file__) + "/../../../../cm116988_BA_Annika_Wierichs/01_LatexDocument/plots/fig_SIM_compactness_Random-Managed"
