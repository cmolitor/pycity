...
desyncPerformances = np.zeros([noHorizons, noOfClusterSplittings])
...
dirResults = "_results_clustering_sortedDSync/{}-{:02d}-{:02d}_{:02d}-{:02d}".format(lt[0], lt[1], lt[2], lt[3], lt[4])
...
########################################################################################################################
    ...
    for s in range(noOfClusterSplittings):
        ...
        # desync
        desyncPerformances[h-startingHorizon, s] = clusterList[s].IDA(fromTime=fromTime, startingBes=0, criterion=criterion, sort='nomPower')

        print "\nDay", h, "\tMembers:", clusterSize_members[s], "\n", desyncPerformances[h-startingHorizon, s]   # for debugging
